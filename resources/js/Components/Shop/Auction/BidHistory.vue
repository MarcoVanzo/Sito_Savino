        ...
        <div v-else class="space-y-2">
            <div
                ...
                :class="[
                    ...
                    bid.is_mine
                        ? 'bg-savino-gold/10 border border-savino-gold/30'
                        : 'bg-gray-800/50 hover:bg-gray-800',
                    index === 0 ? 'ring-1 ring-savino-gold/20' : '',
                ]"
            >
                <div class="flex items-center gap-2 min-w-0">
                    <!-- Crown for highest bid -->
                    <svg v-if="index === 0" class="w-4 h-4 text-savino-gold flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 2l2.5 5 5.5.5-4 4 1 5.5L10 14l-5 3 1-5.5-4-4 5.5-.5L10 2z" />
                    </svg>
                    <span :class="['text-sm font-mono truncate', bid.is_mine ? 'text-savino-gold font-bold' : 'text-gray-300']">
                        {{ bid.bidder }}
                    </span>
                </div>

                <div class="flex items-center gap-3 flex-shrink-0">
                    <span :class="['text-sm font-bold tabular-nums', index === 0 ? 'text-savino-gold' : 'text-white']">
