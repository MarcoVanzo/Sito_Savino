        ...
        <section class="py-16 px-4 sm:px-6 lg:px-8 bg-white">
            <div class="max-w-7xl mx-auto">
                ...
                <div v-if="errorMessage" class="bg-red-50 text-red-600 p-4 rounded-lg mb-4 flex items-center gap-3">
                    <svg aria-hidden="true" class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
                ...
                <div v-if="!isEmpty" class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div class="lg:col-span-2">
                        <!-- Intestazione solo visiva: sotto ogni riga le stesse
                             parole stanno in testo per lo screen reader (md:sr-only). -->
                        <div aria-hidden="true" class="hidden md:grid grid-cols-12 gap-4 pb-4 border-b-2 border-gray-100 text-xs font-bold text-gray-600 uppercase tracking-wider">
                        ...
                        >
                            ...
                            <div class="md:col-span-6 flex items-center gap-4">
                                <div class="w-20 h-20 flex-shrink-0 bg-gray-50 rounded-lg overflow-hidden">
                                    ...
                                    <div v-else class="w-full h-full flex items-center justify-center">
                                        <svg aria-hidden="true" class="w-8 h-8 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <p v-if="item.stock_warning" class="text-xs text-amber-800 font-medium mt-1 flex items-center gap-1">
                                        <svg aria-hidden="true" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                                    ...
                                    <button type="button"
                                        ...
                                        :aria-label="`${$t('shop.remove_from_cart')}: ${item.name}`"
                                        class="md:hidden text-xs text-red-700 hover:text-red-800 mt-1 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                                        {{ $t('shop.remove') }}
                                    </button>
                                </div>
                            ...
                            <div class="md:col-span-2 text-center">
                                <span class="md:sr-only text-xs text-gray-600 mr-2">{{ $t('shop.price') }}:</span>
                                <span class="text-sm font-semibold text-gray-700">{{ formatPrice(item.price) }}</span>
                            </div>

                            <!-- Quantity -->
                            <div class="md:col-span-2 flex items-center justify-center">
                                <span class="md:sr-only text-xs text-gray-600 mr-2">{{ $t('shop.quantity') }}:</span>
                                <div class="inline-flex items-center border border-gray-200 rounded-lg overflow-hidden">
                                    <button type="button"
                                        ...
                                        :aria-label="`${$t('shop.decrease_quantity')}: ${item.name}`"
                                    ...
                                    >
                                        ...
                                        <svg aria-hidden="true" v-else class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" /></svg>
                                    </button>
                                    <span aria-live="polite" aria-atomic="true" class="w-10 h-8 flex items-center justify-center text-sm font-bold text-savino-blue border-x border-gray-200">
                                        {{ item.quantity }}
                                    </span>
                                    <button type="button"
                                        @click="handleUpdateQuantity(item.id, item.quantity + 1)"
                                        :disabled="item.quantity >= (item.disponibili ?? item.stock ?? 99) || loadingItems.has(item.id)"
                                        :aria-label="`${$t('shop.increase_quantity')}: ${item.name}`"
                                    ...
                                    >
                                        <div v-if="loadingItems.has(item.id)" class="animate-spin w-3.5 h-3.5 border-2 border-gray-300 border-t-savino-blue rounded-full"></div>
                                        <svg aria-hidden="true" v-else class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="md:col-span-2 flex items-center justify-end gap-3">
                                ...
                                <button type="button"
                                    @click="handleRemoveItem(item.id)"
                                    :disabled="loadingItems.has(item.id)"
                                    class="hidden md:flex w-8 h-8 items-center justify-center rounded-full text-gray-600 hover:text-red-500 hover:bg-red-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                    :title="$t('shop.remove')"
                                    :aria-label="`${$t('shop.remove_from_cart')}: ${item.name}`"
                                >
                                    <div v-if="loadingItems.has(item.id)" class="animate-spin w-4 h-4 border-2 border-gray-300 border-t-red-500 rounded-full"></div>
                                    <svg aria-hidden="true" v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-2xl p-8 sticky top-32">
                            ...
                            <div v-if="freeShippingThreshold > 0" class="mb-6">
                                <div v-if="hasFreeShipping" class="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-4 py-3">
                                    <svg aria-hidden="true" class="w-5 h-5 text-green-700 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <Link
                                ...
                                class="block w-full text-center bg-savino-blue text-white font-bold uppercase tracking-wider text-sm px-8 py-4 rounded-xl hover:bg-savino-fucsia hover:text-white transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
                            >
                            ...
                            <!-- Security Badges -->
                            <div class="mt-6 flex items-center justify-center gap-4 text-xs text-gray-600">
                ...
                <div v-else class="text-center py-20">
                    <div class="max-w-lg mx-auto">
                        <div class="w-32 h-32 mx-auto mb-8 rounded-full bg-gradient-to-br from-savino-blue to-gray-900 flex items-center justify-center shadow-2xl">
                            <svg aria-hidden="true" class="w-14 h-14 text-savino-fucsia-chiaro" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                        </div>
                        ...
                        <Link :href="route('shop')" class="inline-flex items-center gap-3 bg-savino-blue text-white font-bold uppercase tracking-wider text-sm px-10 py-4 rounded-xl hover:bg-savino-fucsia hover:text-white transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                            <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
                            {{ $t('shop.explore_shop') }}
                        </Link>
                    </div>
                </div>
