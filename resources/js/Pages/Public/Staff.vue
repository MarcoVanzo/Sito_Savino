...
const ogMeta = useOgMeta({
    title: props.page?.title ?? $t('staff.og_title'),
    description: $t('staff.og_description'),
})
    ...
    <PublicLayout>
        ...
        <section class="relative min-h-[40vh] flex items-center justify-center overflow-hidden">
            ...
            <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center py-20">
                <span class="text-savino-fucsia-chiaro text-sm font-bold uppercase tracking-[0.3em]">{{ $t('staff.hero_subtitle') }}</span>
                <h1 class="text-4xl md:text-5xl lg:text-6xl font-black text-white uppercase tracking-tighter mt-4">{{ page?.title ?? $t('staff.og_title') }}</h1>
                <div class="w-16 h-1 bg-savino-fucsia mx-auto mt-4 mb-6"></div>
                <p class="text-white/70 text-lg max-w-2xl mx-auto">{{ $t('staff.hero_description') }}</p>
            </div>
        </section>
        ...
        <section class="py-16 bg-gray-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-20">
                ...
                <div v-if="staffTecnico.length > 0">
                    <h2 class="text-3xl font-black text-savino-blue uppercase tracking-tight mb-8">{{ $t('staff.section_tecnico') }}</h2>
                ...
                <div v-if="staffMedico.length > 0">
                    <h2 class="text-3xl font-black text-savino-red uppercase tracking-tight mb-8">{{ $t('staff.section_medico') }}</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                        <div
                ...
                <div v-if="staffTecnico.length === 0 && staffMedico.length === 0" class="text-center py-20">
                    ...
                    <h3 class="text-2xl font-black text-gray-900 uppercase mb-3">{{ $t('staff.empty_title') }}</h3>
                    <p class="text-gray-500 max-w-md mx-auto">{{ $t('staff.empty_description') }}</p>
                </div>
